...
class Doc {
    ...
    Doc(HashMap doc, String DocName, int N, HashMap freq) {
        ...
        for(int i=0; i<word.size(); i++){
            ...
            System.out.printf("(%d, %.3f) ", wordHash, Math.round(temp.get(DocName).get(wordHash) * 1000) / 1000.0 );
        }
        ...
        for(int i=0; i<N; i++){
            System.out.printf("%d. %s (유사도=%.5f)\n", i+1,DocSim.get(i).doc, Math.round(DocSim.get(i).similarity * 100000)/ 100000.0);
        }
